...
export class DatasetEditManager {
    private static processFilePath(filePath: string): MemberQualifier {
        ...
        const dataSetwithMember = pathArray[pathArray.length - 1].split(SEPARATOR);
        return {
    ...
    public register(subscriptions: vscode.Disposable[], dataProvider: MVSDataProvider) {
        subscriptions.push(
            vscode.commands.registerCommand("zosexplorer.editMember", async (arg) => {
                try {
                    ...
                    vscode.window.showErrorMessage("Edit member error: " + error);
                }
            }),
        );
        subscriptions.push(
            vscode.workspace.onDidChangeTextDocument((event: vscode.TextDocumentChangeEvent) => {
                if (
                    ...
                    this.markEditedMember(DatasetEditManager.processFilePath(event.document.fileName))
                ) {
        ...
        subscriptions.push(
            vscode.workspace.onDidCloseTextDocument((closedDocument: vscode.TextDocument) =>
                this.closeDocument(closedDocument),
            ),
        );
        subscriptions.push(
            vscode.workspace.onWillSaveTextDocument(async (saveEvent: vscode.TextDocumentWillSaveEvent) => {
    ...
    public async saveToMainframe(filePath: string): Promise<boolean> {
        const message = await vscode.window.showWarningMessage("Do you want to save changes to Mainframe?", "Save");
        if (message === "Save") {
            try {
    ...
    public isEditedMember(hostName: string, dataSetName: string, memberName?: string): boolean {
    ...
    public cleanEditedMember(host: Connection, dataSet: Dataset, member: Member): boolean {
    ...
    public unmarkMember(hostName: string, datasetName: string, memberName: string) {
        this.unmarkEditedMember({ datasetName, hostName, memberName });
    }

    private async editMember(host: Connection, dataset: Dataset, member: Member, dataProvider: MVSDataProvider) {
        const content = await this.rest.getContent(host, `${dataset.name}(${member.name})`);
        const filePath: string = generateTempFileName(host, dataset, member);
        ensureDirectoryExistence(filePath);
        if (fs.existsSync(filePath) && fs.readFileSync(filePath, DEFAULT_ENCODING) !== content) {
    ...
    private async saveDocument(savedDoc: vscode.TextDocument, dataProvider: MVSDataProvider) {
        const memberQualifier: MemberQualifier = DatasetEditManager.processFilePath(savedDoc.fileName);
        if (
            this.isEditedMember(memberQualifier.hostName, memberQualifier.datasetName, memberQualifier.memberName) &&
    ...
    private closeDocument(closedDocument: vscode.TextDocument) {
        ...
        const memberQualifier: MemberQualifier = DatasetEditManager.processFilePath(fileName);
        if (
            !this.isEditedMember(memberQualifier.hostName, memberQualifier.datasetName, memberQualifier.memberName) &&
    ...
    private markEditedMember(memberQualifier: MemberQualifier): boolean {
        ...
        if (!editedMembers[memberQualifier.datasetName]) {
            ...
            editedMembers[memberQualifier.datasetName].add(memberQualifier.memberName);
            return true;
        }

        if (!editedMembers[memberQualifier.datasetName].has(memberQualifier.memberName)) {
            editedMembers[memberQualifier.datasetName].add(memberQualifier.memberName);
            return true;
        }
    ...
    private unmarkEditedMember(memberQualifier: MemberQualifier) {
        if (!this.isEditedMember(memberQualifier.hostName, memberQualifier.datasetName, memberQualifier.memberName)) {
            return;
        }

        this.editedMemList[memberQualifier.hostName][memberQualifier.datasetName].delete(memberQualifier.memberName);
        if (this.editedMemList[memberQualifier.hostName][memberQualifier.datasetName].size === 0) {
            delete this.editedMemList[memberQualifier.hostName][memberQualifier.datasetName];
        }
        if (Object.keys(this.editedMemList[memberQualifier.hostName]).length === 0) {
    ...
    private async uploadtoMainframe(filePath: string) {
        ...
        const memberQualifier: MemberQualifier = DatasetEditManager.processFilePath(filePath);
        const host: Connection | undefined = SettingsFacade.findHostByName(
            memberQualifier.hostName,
            SettingsFacade.listHosts(),
        );
        if (host === undefined) {
            throw new Error(`hostName ${memberQualifier.hostName} is not define`);
        }
        await SettingsFacade.requestCredentials(host);
        await this.rest.putContent(host, content, memberQualifier.datasetName, memberQualifier.memberName);
    }
}
